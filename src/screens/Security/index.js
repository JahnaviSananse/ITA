...
class Security extends Component {
  ...
  componentWillMount() {
    ...
    TouchID.isSupported(optionalConfigObject)
      .then(biometryType => {
        // Success code
        if (biometryType === 'FaceID') { // for ios
          this.setState({
            isSupportedTouchId: 2
          })

        } else if (biometryType === 'TouchID') { // for ios
          this.setState({
            isSupportedTouchId: 1
          })
          //this.allowedSupport(language.ActivateTouchIDAlert, '1')
        } else if (Platform.OS === 'android' && biometryType === true) { // for android
          this.setState({
            isSupportedTouchId: 1
          })
        }
        ...
        <View style={{ flex: 1, backgroundColor: utility.changeHeaderColor('#FFFFFF') }}>
          ...
          <View>
            ...
            <TouchableOpacity onPress={() =>
              ...
              <FastImage style={styles.iconNext} source={this.state.isSupportedTouchId === 0 ? null : utility.changeNextButton()} />
            </TouchableOpacity>
          </View>
          {/* <FlatList
            data={this.data}
