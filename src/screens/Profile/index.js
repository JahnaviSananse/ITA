...
import { View, Text, Alert, SafeAreaView, Image, AsyncStorage } from 'react-native';
...
import { getFavList, getRecentList, getRecentPullToRefresh, getFavListPull } from '../../store/RecentFav/actions';
	...
	constructor() {
		...
		this.state = {
			...
			favdata: {
				...
				description: 'Whenever you see this icon, tap it! Everything will be saved here.',
			...
			pdfData: [ { title: 'Evolution' }, { title: 'S&P 500 Index' }, { title: 'Platinum' } ]
		};
		setTimeout(() => {
			this.getData();
		}, 200);
	...
	render() {
		...
		return (
			<SafeAreaView style={{ flex: 1, backgroundColor: utility.changeHeaderColor('#F3F3F3') }}>
				<View style={{ flex: 1, backgroundColor: utility.changeBackgroundColor('#FFFFFF'), paddingBottom: 50 }}>
					<TabHeader
						title={language.Profile}
						img={utility.changeSettingButton()}
						backgroundColor={utility.changeHeaderColor('#F3F3F3')}
						redirectRight={() => this.props.navigation.push(this.state.redirectScreen)}
					/>
					<ScrollableTabView
...
const mapStateToProps = (state) => {
	const { loading, isSuccess, recentList, favoriteList, pullToRefresh } = state.recentfav;
