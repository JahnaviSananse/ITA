...
import { Image, Dimensions, View, ScrollView, SafeAreaView, Text, StatusBar, TouchableOpacity } from 'react-native';
...
class Filter extends Component {
    ...
    setCurrentValue() {
        const { assestClass, investmentUniverse, fundFamily } = this.props.navigation.state.params
        ...
        let investmentUniverseLocal = JSON.parse(JSON.stringify(fundCat['investment-universe']))
    ...
    componentWillUnmount() {
        if (this.state.isUpdate) {
            ...
            if (this.props.navigation.state &&
                ...
                this.props.navigation.state.params.callback(finalObject);
            }
        }
    }
    onClick(index) {
        utility.recordEvent("Filter: Option selection from dropdown(Investment universe)")
        ...
        return (
            <View
                ...
                <TouchableOpacity
                    ...
                    <Text style={{ color: utility.changeFontColor('#233746'), paddingLeft: 15 }}>{language.Done}</Text>
                </TouchableOpacity>
                <Text style={[styles.text, { color: utility.changeFontColor('#233746') }]}>{language.Filter}</Text>
                <TouchableOpacity onPress={() => this.clearFilterData()}
                    style={styles.ButtonTouch}>
                    <Text style={{ color: utility.changeFontColor('#233746'), paddingRight: 15 }}>{language.Clear}</Text>
                </TouchableOpacity>
            </View >
        );
    }
        ...
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: utility.changeHeaderColor('#F3F3F3') }}>
                <View style=
                    ...
                    /> */}
                    {this.renderHeader()}
                    <ScrollView style={{ backgroundColor: utility.changeBackgroundColor("transparent") }}>
                        ...
                        <View style={{ width: '100%', }}>
                            <TouchableOpacity
                                ...
                                <Text style=
                                    ...
                                    {language.InvestmentUniverse}</Text>
                                {this.state.expanded ?
                                    < Image source={utility.changeAccordianupButton()} style={styles.iconStyle} resizeMode={'contain'} /> :
                                    <Image source={utility.changeAccordianDownButton()} style={styles.iconStyle} resizeMode={'contain'} />
                                }
                            </TouchableOpacity>
                            {this.state.expanded &&
                                <View style={styles.countryContainer}>
                                    {
                                        this.state.data[1].data.map((value, index) => {
                                            return (
                                                <TouchableOpacity
                                                    style={[styles.countrySelection, {
                                                        backgroundColor: value.value == false ? '#FFFFFF' : '#8cb4b6',
                                                    }]}
                                                    onPress={() => {
                                                        this.onClick(index)
                                                    }} >
                                                    <Text style={{ color: value.value == false ? '#6a6a6a' : '#FFFFFF' }}>{value.name}</Text>
                                                </TouchableOpacity>
